...
//    labelGold = [[UILabel alloc]init];
    ...
    collectArr = @[@{@"icon":@"23-2",@"title":@"个人资料"},@{@"icon":@"23-3",@"title":@"绑定手机"},@{@"icon":@"23-4",@"title":@"我的车友"},@{@"icon":@"23-5",@"title":@"专属代表"},@{@"icon":@"23-6",@"title":@"功能设置"},@{@"icon":@"23-7",@"title":@"我的收藏"}];
...
-(void)getUserInfomation{
    ...
    [manager GET:[NSString stringFollowBaseUrl:getUserInfo] parameters:nil progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if (self.resultDic.count>0) {
            ...
            [[NSUserDefaults standardUserDefaults]setValue:self.resultDic[@"avatar"] forKey:@"iconImagView"];
...
//        Lichen.sd_layout.topSpaceToView(labelRank,30).leftSpaceToView(headViewback,80).heightIs(35).widthIs(90);
        Lichen.sd_layout.topSpaceToView(labelRank,30).centerXEqualToView(self.view).heightIs(35).widthIs(90);
    }else{
//        Lichen.sd_layout.topSpaceToView(labelRank,20).leftSpaceToView(headViewback,60).heightIs(35).widthIs(90);
        Lichen.sd_layout.topSpaceToView(labelRank,20).centerXEqualToView(self.view).heightIs(35).widthIs(90);
    }
