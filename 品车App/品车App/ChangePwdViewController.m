...
@interface ChangePwdViewController ()<MBProgressHUDDelegate>
{
...
- (void)viewDidLoad {
    ...
    passwordField.leftView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, 8, 0)];
    ...
    [createBtn addTarget:self action:@selector(changePassWord) forControlEvents:UIControlEventTouchUpInside];
...
-(void)changePassWord
{
    ...
    NSDictionary *param = @{@"mobile":phoneField.text,@"verifycode":codeField.text,@"password":passwordField.text};
    [manager POST:[NSString stringFollowBaseUrl:resetpasswordURL] parameters:param progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        progressHUD.hidden = YES;
        
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        progressHUD.label.text  = @"修改失败，请重试！";
